<template>
  <Content>
    <Bar
      id="back"
      title="リサーチ / PEST"
      color="hover:text-teal-500"
    />
    <CardImage class="col-span-2">
      <img src="~assets/images/pest.gif" width="2560" height="1440" alt="POWWWS Laboratory — PEST"/>
    </CardImage>
    <CardText class="col-span-2">
      <HeadingSans>
        リサーチ
      </HeadingSans><br />
      <HeadingSerif class="text-5xl flex">
        PEST
        <div class="ml-2 mt-1 w-8 h-8 rounded-full bg-red-500 flex items-center justify-center">
          <svg class="w-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
            <path d="M288 32c0-17.7-14.3-32-32-32s-32 14.3-32 32V43.5c0 49.9-60.3 74.9-95.6 39.6L120.2 75C107.7 62.5 87.5 62.5 75 75s-12.5 32.8 0 45.3l8.2 8.2C118.4 163.7 93.4 224 43.5 224H32c-17.7 0-32 14.3-32 32s14.3 32 32 32H43.5c49.9 0 74.9 60.3 39.6 95.6L75 391.8c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0l8.2-8.2c35.3-35.3 95.6-10.3 95.6 39.6V480c0 17.7 14.3 32 32 32s32-14.3 32-32V468.5c0-49.9 60.3-74.9 95.6-39.6l8.2 8.2c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3l-8.2-8.2c-35.3-35.3-10.3-95.6 39.6-95.6H480c17.7 0 32-14.3 32-32s-14.3-32-32-32H468.5c-49.9 0-74.9-60.3-39.6-95.6l8.2-8.2c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0l-8.2 8.2C348.3 118.4 288 93.4 288 43.5V32zM176 224a48 48 0 1 1 96 0 48 48 0 1 1 -96 0zm128 56a24 24 0 1 1 0 48 24 24 0 1 1 0-48z"/>
          </svg>
        </div>
      </HeadingSerif>
    </CardText>
    <CardDetail class="col-span-2">
      <div class="grid grid-cols-1 sm:grid-cols-2 py-6 gap-12">
        <div class="space-y-4">
          <span class="block text-white">POWWWS研究所で行われているPEST研究は、血液感染症と疾患に焦点を当てています。私たちの科学者チームは、人間の健康に重大な脅威をもたらすさまざまな血液感染症の原因、伝播、治療を理解することに専念しています。血液サンプルの詳細な分析と先進的な分子技術を用いることで、これらの感染症の原因となる特定の病原体を特定し、その作用機序を解明し、診断と治療のための効果的な戦略を開発することを目指しています。</span>
        </div>
        <div class="space-y-4">
          <span class="block text-white">私たちの研究は、血液感染症の理解を深めるだけでなく、これらの感染症と戦うための新しい治療法や予防策の開発に貴重な示唆を提供しています。</span>
        </div>
      </div>
    </CardDetail>
    <Address />
  </Content>
</template>
<script>
  export default {
    mounted() {
      this.$nextTick(() => {
      })
    },
    beforeRouteLeave(to, from, next) {
      setTimeout(function() {
        next()
      }, 173)
    }
  }
</script>

